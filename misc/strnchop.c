 ...
 *
 * prototype:
char *strnchop(const char *string, size_t *string_length, int chop_len,
 ...
 *	string_length	= length of string to consider or returns the
 *			  length of what's left.  Does not assume
 ...
 *
 *	strnchop	= returns a pointer to an internal area that contains
...
/* if first time with this string, dealloc any memory & reallocate space */
	if (string != (char *) NULL) {
		...
		if ((chop_buffer = malloc(chop_len + 1)) == (char *) NULL) {
			/* memory error */
#ifdef RKOERROR
			(void) rkocpyerror("strnchop : malloc error");
			rkoerrno = RKOMEMERR;
#endif
			return (char *) NULL;
		}
...
#ifdef RKOERROR
			(void) rkocpyerror(
			"strnchop : user error - buffer was deallocated");
			rkoerrno = RKOUSEERR;
#endif
			return (char *) NULL;
	}
/* strip initial spaces */
	while ((isspace((int) *start)) && (*string_len >=0)) {
		start++;
		*string_len;
	}
...
#ifdef RKOERROR
			(void) rkocpyerror(
				"strnchop : user error - string can not be "
...
void testfn(const char *test, int len) {
	...
	(void) printf(
		...
		"1234567890123456789012345678901234567890\n");
	do {
		ptr = strnchop(ptr, len, &str_len, &next);
		(void) printf("%s: => %d\n", ptr, str_len);
		ptr = (char *) NULL;
...
void testfn2(const char *test, int begin, int len1, int len2) {
	...
	ptr = test;
	do {
		ptr = strnchop(ptr, len1, &str_len, &next);
		(void) printf("%s: => %d\n", ptr, str_len);
		ptr = (char *) NULL;
	...
	if (next != (char *) NULL) {
		ptr = next;
		do {
			ptr = strnchop(ptr, len2, &str_len, &next);
			(void) printf("%s: => %d\n", ptr, str_len);
			ptr = (char *) NULL;
		} while (next != (char *) NULL);
	}
...
int main() {
	char *test =
"This is a test of some long text to see how well strnchop can do in "
...
"many as 8 spaces sent to a terminal screen.";
	...
	tmp = strnchop(test2, 10, NULL, NULL);
	if (tmp == (char *) NULL) {
#ifdef RKOERROR
		if (rkoerrno != RKO_OK) {
			rkoperror("OK - main");
	...
	tmp = strnchop(test2, 40, NULL, NULL);
	if (tmp == (char *) NULL) {
#ifdef RKOERROR
		if (rkoerrno != RKO_OK) {
			rkoperror("FAIL - main");
	...
	tmp = strnchop(test2, -1, NULL, NULL);
	if (tmp == (char *) NULL) {
#ifdef RKOERROR
		if (rkoerrno != RKO_OK) {
			rkoperror("FAIL - main");
	...
	tmp = strnchop(NULL, 40, NULL, NULL);
	if (tmp == (char *) NULL) {
#ifdef RKOERROR
		if (rkoerrno != RKO_OK) {
			rkoperror("OK - main");
